	...
	if ( socket.handshake.headers.referer == "https://ccs.sanofi-mobile.com.br/" ){
        ...
        socket.on('sentinel_monitor', function (payload) {
                ...
                dbcc.query("SELECT uuid() as UUID;", function (err, id) {
                        var _custom_uid = id[0].UUID;
                        if (_host == "LON") {
                                request.post({ url: 'https://www.waboxapp.com/api/send/chat', form: { token: 'f5f56af0e668306ccb81bee06fdbe1425a87473b5e615', uid: '5511995518459', to: '5511999713755', custom_uid: _custom_uid, text: payload } }, function (err, httpResponse, body) {
                                        log("Response WABOXAPP Monitor", body);
                                });
                        }
                });
                                                ...
                                                function (err, result) {
                                                        if (err) {
                                                                ...
                                                                var dataSet = {
                                                                        mobile: _mobile,
                                                                        type: "chat",
                                                                        message: "Ola, sou " + _fkname + " e gostaria de conversar com voce sobre a nossa plataforma!"
                                                                };
                                                                //sendMyChat(dataSet);
                        ...
                        dbcc.query("SELECT COUNT(*) AS resultCount FROM tab_ativo WHERE dtcadastro BETWEEN '" + d3.toISOString() + "' AND '" + d1.toISOString() + "' AND STATUS = '0';", [], function (err, result) {
                                if (result[0].resultCount < 300) {
                                        ...
                                        dbcc.query("SELECT UUID() AS UUID;", [], function (err, result) {
                                                ...
                                                dbcc.query("INSERT INTO db_sanofi_ccs.tab_ativo (id, tipo, nome, nomefantasia, mobile, base, bandeira, uf, filename) VALUES (?,?,?,?,?,?,?,?,?);", [_id, _tipo, _nome, _fantasia, _mobile, _base, _bandeira, _uf, _filename], function (err, result) {
                                                        if (err) { console.log(err); }
